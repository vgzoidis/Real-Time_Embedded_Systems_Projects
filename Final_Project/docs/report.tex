...
To rigorously test the self-healing telemetry logger while preserving the SSH trace, we simulated WAN constraints by leveraging kernel-level \texttt{iptables}. 
By dynamically dropping external TCP traffic destined specifically for the Firehose architecture (\texttt{sudo iptables -A OUTPUT -p tcp -d jetstream1.us-east.bsky.network -j DROP}), the SSH session remained securely established on the Local Area Network (LAN). The \texttt{libwebsockets} thread correctly caught the simulated WAN severance, emitted an error trace, and cleanly transitioned into its 3-second redial loop. Upon executing the \texttt{iptables} removal rule, the C-backend instantly restored the WebSocket context, recovering the inbound data stream indefinitely without requiring a system restart.
