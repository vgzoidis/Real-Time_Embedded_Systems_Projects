...
\subsection{Data Persistence and System Autonomy (fdatasync and Systemd)}
Ensuring the integrity of the collected metrics across an unprompted power failure or operating system crash was a primary requirement for a continuous 24-hour run. Standard C I/O buffers (\texttt{fflush(fp)}) only transport data to the Linux kernel's page cache. In the event of an abrupt reboot, the page cache is destroyed, leading to lost or mangled data rows in the log. To achieve true persistence, the Monitor thread invokes \texttt{fdatasync(fileno(fp))} after every write operation, forcing the Linux kernel to physically commit the page block to the SD card.

To provide seamless fault recovery without human intervention, the application was wrapped in a \texttt{systemd} service (\texttt{telemetry.service}). This service employs a \texttt{Restart=always} policy, ensuring that if the Raspberry Pi experiences an unexpected reboot or Out-Of-Memory (OOM) termination, the logger service is automatically reinstated. The combination of absolute time alignment, SD card physical flushing, and \texttt{systemd} supervision allows the device to operate completely autonomously and defensively throughout the mandatory 24-hour data logging timeframe.
...
As visualized, the jitter is tightly bounded (execution is highly deterministic, with the median and mean average jitter resting perfectly at 0.00000 ms, and the 75th percentile reaching merely 0.01101 ms).

\subsection{Network Burstiness vs. Buffer Occupancy}
As recorded during the preliminary Firehose probing, network ingestion is irregular. To visualize the "burstiness" of the network and the efficiency of the queue, the incoming message frequency is superimposed against buffer usage in Figure \ref{fig:burstiness}.
...
The orange trajectory illustrates violent network bursts frequently exceeding 150 Hz, with peak transients crossing 350 Hz. Despite this, the green buffer occupancy metric remains mathematically near zero, recording a peak occupancy of only 0.78\%. This empirically proves the Consumer thread parses data magnitudes faster than maximum network ingress.
...
Figure \ref{fig:cpu} demonstrates excellent thermal and processing headroom. While momentary maximum CPU loads can spike to 100.00\% during the most intense network events, the application maintains a highly efficient average CPU load of just 10.39\% across the entire day. 
...
\section{Tools Used and Sources}

This project combined embedded C development, network probing, and post-processing analysis tools to validate both correctness and long-run stability.

\subsection{Tools Used}
\begin{itemize}
  \item \textbf{C toolchain:} \texttt{make}, \texttt{gcc}, \texttt{pthread}, and the Raspberry Pi ARM cross-compilation toolchain.
  \item \textbf{Networking stack:} \texttt{libwebsockets} for the WebSocket producer and \texttt{iproute2} for outage simulation.
  \item \textbf{JSON parsing:} \texttt{cJSON} for decoding Jetstream payloads.
  \item \textbf{Validation and profiling:} \texttt{valgrind} for leak checking and \texttt{systemd} for service supervision.
  \item \textbf{Python analysis:} \texttt{probe\_firehose.py} and \texttt{analyze\_metrics.py}, supported by \texttt{pandas}, \texttt{matplotlib}, and \texttt{numpy} from \texttt{requirements.txt}.
\end{itemize}

\subsection{Sources}
\begin{itemize}
  \item Bluesky Jetstream Firehose endpoint: \url{wss://jetstream1.us-east.bsky.network/subscribe}.
  \item Raspberry Pi cross-compiler toolchain: \url{https://sourceforge.net/projects/raspberry-pi-cross-compilers/files/Raspberry%20Pi%20GCC%20Cross-Compiler%20Toolchains/Buster/GCC%208.3.0/Raspberry%20Pi%201%2C%20Zero/}.
  \item Official \texttt{libwebsockets} project documentation and API references.
  \item \texttt{cJSON} project documentation and source code.
  \item Linux manual pages for \texttt{clock\_nanosleep()}, \texttt{fdatasync()}, \texttt{pthread\_cond\_wait()}, and \texttt{systemd} service management.
  \item Local experiment logs and outputs: \texttt{metrics\_log.txt}, \texttt{telemetry\_logger.out}, and the generated statistics summary.
\end{itemize}
